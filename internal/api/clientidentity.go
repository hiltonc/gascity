...
// sanitizeHeaderValue bounds each User-Agent component and strips anything
// that could terminate or forge a header line. The subcommand is derived from
// argv, so it is untrusted input; sanitizeAuditString is the one place that
// owns the control-character rule, here and on the logging side.
